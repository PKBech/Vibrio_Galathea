...
ANIb_dist_long_meta <- df_Vibrio %>% select(genome_id, CTDTemperature, CTDSalinity) %>% left_join(ANIb_dist_long, df_Vibrio, by="genome_id") 
...
colnames(ANIb_dist_long_meta)[4] <- "genome_id"

ANIb_dist_long_meta <- df_Vibrio %>% select(genome_id, CTDTemperature, CTDSalinity) %>% left_join(ANIb_dist_long_meta, df_Vibrio, by="genome_id") 
...
ANIb_dist_long_meta %>% 
  ggplot(aes(sal_diff, ANI_similarity)) +
